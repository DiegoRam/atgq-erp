...
1. **`npm run lint`** y **`npx tsc --noEmit`** must pass clean. Ninguno de los dos escribe en `.next/`, así que son seguros con el dev server levantado — por eso el `npm run build` completo va al final (paso 5) y no acá.
...
5. **`npm run build`** must pass clean — y va **último, después del browser**, no primero. `next build` y `next dev` comparten el directorio `.next/`: correr el build con un dev server levantado le pisa los manifests y lo deja sirviendo `500` con `ENOENT: .next/prerender-manifest.json`, con lo cual el paso 4 ya no se puede hacer. Dos secuelas que confunden:
   ...
   - Si el build compila pero falla en "Collecting page data" (`Cannot find module for page: /_not-found`), es la misma carrera contra el dev server escribiendo en paralelo: `rm -rf .next` y reintentar.
...
- **Todo selector de un ítem — `items_ventas` o `stock_items` — se hace con combobox buscable, nunca con un `<Select>` plano**: `ItemVentaCombobox` / `StockItemCombobox` (sobre `shared/Combobox`), que cargan el catálogo entero una vez y filtran en el cliente por nombre, descripción y stock vinculado. Con cientos de ítems una lista sin buscador es inusable, y vale igual para altas, filtros y reportes.
- Las acciones que alimentan esos combos traen **todas** las filas con `fetchAllRows()` (`src/lib/supabase/fetch-all-rows.ts`): un `.select()` sin `.range()` corta en 1000 filas en silencio, y un catálogo truncado esconde ítems que sí existen.
