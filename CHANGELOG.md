...
- **Padrón electoral** (P13.1, migración `20260817000001`). `/socios/padron` gana un toggle **"Solo habilitados a votar"** que aplica sobre el listado, el export y la hoja impresa. Habilitado = categoría con derecho a voto + 18 años cumplidos + 1 año de antigüedad + al día de cuota social, todo contra `CURRENT_DATE`. En el seed local: 45 socios activos → 35 por categoría → 33 por edad → **28 habilitados**.
  - **Las dos reglas de negocio son banderas editables, no nombres hardcodeados en la query.** `categorias_sociales.habilita_voto` y `tipos_cuotas.afecta_padron`, con seed por nombre una sola vez, igual que `cuenta_como_activo` (`20260806000001`). Los nombres de categoría vienen migrados del legacy y se editan desde el ABM: un typo corregido desde la UI rompería el padrón en silencio, sin error, hasta el día de la asamblea. Ambas van en `DEFAULT false` —al revés que `cuenta_como_activo`— porque un padrón electoral **falla cerrado**: una categoría creada mañana no otorga voto por omisión. Los dos seeds abortan la migración con `RAISE EXCEPTION` si no matchean ninguna fila, en vez de aplicarse "OK" y dejar el criterio apagado.
  - **Sólo la cuota social inhabilita, y sólo la de períodos anteriores al último emitido.** La del último período se tolera impaga porque puede no haber vencido. La distinción por tipo no es cosmética: los 48 socios del seed arrastran una Cuota Actividad impaga de marzo, así que contar todos los tipos dejaba el padrón en **0**.
  - **El corte está acotado al mes en curso, y ese tope es el arreglo de un bug con dientes.** `v_corte` es global al club, así que **una** cuota social con período futuro —un typo de año en el input libre de `socios/cuotas/generar`, o una emisión adelantada— movía el corte de todos: medido sobre el seed, una sola cuota en `2030-01` hacía caer el padrón de **28 a 11 habilitados (–61%)**, sin ningún error. Con el tope, esa misma fila no mueve nada. Deliberadamente **no** es un corte por socio: sería más robusto todavía (28 con datos limpios, 27 con el typo), pero cambia la semántica que el club aprobó y abre otro agujero — un socio con una única cuota vieja impaga pasaría a estar habilitado.
  - **Los criterios son auditables desde la pantalla, no sólo desde el SQL.** El período de corte se muestra en pantalla ("Al día hasta 03/2026 inclusive"), hay un banner cuando ningún tipo de cuota tiene `afecta_padron` —apagarlo desactiva el criterio de deuda por completo y antes no se veía en ningún lado— y una línea cuenta los habilitados **sin cuotas emitidas**, que pasan el `NOT EXISTS` por ausencia de datos y no por haber pagado (3 en el seed, los Vitalicios). La hoja impresa del padrón general declara fecha de emisión, total, filtros y criterios aplicados; la del padrón electoral va mínima (ver «Padrón electoral: la hoja impresa se publica» más abajo).
...
- **Precio diferenciado para socios y no socios en los ítems de venta** (migración `20260812000001`). `items_ventas` pasa de un precio a dos: `precio` (que ahora significa explícitamente *tarifa de socio*) y `precio_no_socio`. El toggle **Socio | No Socio** que ya existía en el POS pasa de sólo cambiar qué datos del comprador se piden a **determinar cuánto se cobra**.
  ...
  - **Togglear Socio/No Socio reprecia el carrito ya cargado** y avisa con un toast. Sin eso el cajero cotizaba un total y la base cobraba otro, porque el precio del carrito se fijaba al agregar el ítem. El diálogo de venta registrada pasa a mostrar el `venta_total` que devuelve la RPC en vez del total del carrito: con dos tarifas una divergencia cliente/servidor es materialmente posible (catálogo stale), y conviene que se vea en vez de quedar silenciosa.
  - En el ABM, el precio de no socio **se autocompleta a socio + 20% hasta que alguien lo edita**; ahí queda manual y deja de seguirlo, con un link "Volver a socio + 20%" para reengancharlo. Al abrir un ítem se deduce cuál de los dos casos es comparando el par guardado contra la regla, de modo que los ítems con tarifa propia del legacy no se pisan. El enganche se corta con el `onChange` de `register` y no con `touchedFields`, que sólo se prende en el blur: tipear sin salir del campo habría dejado que la próxima tecla en el precio de socio pisara lo cargado.
  - **`precio` pasa de `.positive()` a `.nonnegative()` en Zod.** Los 21 ítems legacy con precio de socio $0 son una tarifa real, no un dato faltante, y con `.positive()` el ABM no dejaba guardarlos ni para corregirles el nombre — bloqueaba exactamente los ítems cuya tarifa de no socio hay que poder cargar. La validación nunca fue un invariante real: `itemVentaSchema` tiene un solo consumidor, las acciones no validan del lado servidor y la base no tiene CHECK.
  - La columna queda `NOT NULL` y **sin `DEFAULT`** a propósito: un `DEFAULT 0` silencioso convertiría un INSERT olvidado en un ítem gratis en vez de fallar fuerte. La contra es que hay que **aplicar la migración junto con el deploy del frontend**, no antes, o dar de alta un ítem falla con `23502` en esa ventana. `migration/migrate.py` se actualizó para insertar ambas tarifas.

- **Recargo por defecto del 20% para no socios y visibilidad de la tarifa en el POS** (migración `20260812000002`). Con las tarifas legacy recuperadas, **165 de los 185 ítems activos quedaron con `precio_no_socio = precio`**: el club nunca cargó la segunda tarifa en el sistema viejo. En el mostrador eso se lee como un toggle roto — se elige *No Socio* y el importe del carrito no se mueve, porque las dos tarifas son el mismo número (`Blanco Fusil 1 Zona`: $1.500 y $1.500). No era un bug de código: el POS, la RPC y el combobox ya elegían bien la tarifa.
  - La migración aplica `round(precio * 1.2, 2)` sobre **158 filas**, con tres guards que definen qué queda afuera. `precio > 0` excluye los 19 ítems activos donde el `ValorSocio = 0` del legacy significaba "esto no se le vende a socios" (21 contando los inactivos): ahí la tarifa de no socio ya es la correcta y multiplicar cero no aporta nada. `activo` excluye los ~24 ítems "(DESUSO)" que desactivó `20260808000001` — no se venden, y ésta es una columna de plata: cuantas menos filas mueva, mejor. Y `nombre !~* 'socio'` excluye los ítems donde **el club modeló la distinción como filas separadas en vez de dos tarifas**: `Permiso de Caza - No Socio` ($400) y `Permiso de Caza - Socio` ($330) son dos ítems, cada uno con sus dos tarifas iguales; sin ese guard el no socio pagaría $480 por lo que el tarifario cobra $400, con el recargo aplicado dos veces —una en el nombre y otra en la columna—. Tampoco se tocan los 20 ítems cuya tarifa de no socio ya difiere: son precios reales del club.
  - **No es re-runnable-safe, y es a propósito.** El `WHERE` no distingue "la segunda tarifa nunca se cargó" de "el club decidió cobrar lo mismo a todos", así que una segunda corrida pisaría esa decisión. Lo que garantiza la única pasada es el ledger de migraciones, no el predicado. La cabecera del archivo documenta el `UPDATE` inverso para revertir, con su salvedad: al no quedar marca de qué filas se tocaron, el inverso se re-deriva de la regla y alcanza también a lo que el ABM haya dejado en socio + 20%.
  - **El carrito nombra la tarifa que está aplicando** ("Precio (Socio)" / "Precio (No Socio)") y el buscador de ítems muestra **las dos**, con la vigente resaltada (`$ 300,00 / $ 360,00`) y una sola cuando coinciden. El toggle sigue repreciando en silencio si ninguna difiere, pero ahora el cajero ve *por qué* el importe no cambió en vez de asumir que la pantalla falla.
  - **Aviso por ítems en $0.** Los 19 ítems con `precio = 0` (Derecho de línea, Idoneidad de tiro, ESTACIONAMIENTO, CHALET) hoy un socio los compraría gratis, y hay 4 más con **ambas** tarifas en $0 (ALQUILER ARMERIA, PASE DE TESORERIA) que se venden gratis a cualquiera. La línea se marca en rojo con "sin tarifa de socio"/"de no socio" según el toggle, y aparece una nota bajo la tabla. No bloquea la venta: si esos ítems se cobran o no es decisión del club, no del sistema.

- **El recargo para no socios pasa a ser un parámetro del sistema, con reconstrucción masiva del tarifario** (migración `20260812000003`, pantalla `/security/configuracion`). Hasta acá el 20% era un literal en `ItemVentaForm.tsx` y en dos migraciones: cambiarlo exigía tocar código y desplegar, y no había forma de reaplicarlo al catálogo existente. Ahora el porcentaje se edita desde Security → Parámetros del Sistema y un proceso recorre **todos** los ítems recalculando `precio_no_socio = round(precio * (1 + pct/100), 2)` a partir de la tarifa de socio.
  - **Tabla `configuracion` de una sola fila, no key/value.** Es la réplica moderna de la `Configuracion` del legacy (`docs/backup_schema.sql:39-68`), cuyo `VentaDefault = 20` era exactamente este parámetro y no se había portado. Un `valor text` genérico habría sacado la validación de la base y obligado a parsear en cada lectura; con columnas tipadas el rango vive en un `CHECK (>= 0 AND <= 200)` y agregar el próximo parámetro (SMTP, `StockNegativo`, datos de la entidad) es un `ADD COLUMN` de una línea. El singleton lo garantiza `id INTEGER PRIMARY KEY DEFAULT 1 CHECK (id = 1)`, así que todo lector hace `where id = 1` sin preguntarse cuál fila es la buena. El tope de 200% es un guard de fat-finger: un `2000` tipeado en lugar de `20` triplicaría el tarifario entero de un click. Un recargo de 0 es válido y significa "el no socio paga lo mismo".
  - **Alcance: todos los ítems, sin excepción** — inactivos incluidos, y pisando las 20 tarifas propias que rescató `20260812000001`. Es lo pedido, y es lo que hace que el parámetro sea la única fuente de verdad, pero también es destructivo, así que el preview **muestra las dos consecuencias en vez de esconderlas**: cuántos ítems con `precio = 0` quedarán también en $0 para no socios perdiendo su tarifa real, y cuántos ya distinguen socio/no socio en el nombre (`Permiso de Caza - No Socio`) y recibirán el recargo por segunda vez. Son los mismos casos que `20260812000002` había excluido con guards; acá se informan y se aplican.
  - **Preview y ejecución son la misma función**, `recalcular_precios_no_socio(p_dry_run)`, y el porcentaje **no es parámetro**: sale de `configuracion` adentro del RPC. Así el browser no puede mandar el suyo, ni el preview calcular con un número y la escritura con otro. Las filas a tocar salen de un único helper `precios_no_socio_pendientes(pct)` que consumen el conteo, la muestra de 50 filas y el `UPDATE`, que es lo que impide que la previsualización prometa una cosa y la corrida haga otra. El `IS DISTINCT FROM` del helper no acota el alcance: evita `UPDATE`s que no cambian nada y deja la corrida idempotente (la segunda vez reporta 0 afectados).
  - **La cuenta se hace en centavos enteros del lado de JS.** `Number((precio * 1.2).toFixed(2))` es aritmética binaria y `round(numeric)` en Postgres es exacta con redondeo half-away-from-zero: con `precio = 1,15` y 50%, PG escribe `1,73` y `toFixed` sugería `1,72`. El formulario y la reconstrucción masiva se habrían contradicho por un centavo en los ítems donde el redondeo cae justo. `precioNoSocioSugerido` queda en `src/lib/precios.ts`, compartida por el ABM, y verificada contra la base en 357 combinaciones de precio y porcentaje.
  - **Autorización cruzada.** La pantalla vive en el módulo seguridad pero la escritura cae sobre el tarifario, así que el RPC —`SECURITY DEFINER`, porque escribe `items_ventas` sorteando RLS— exige `seguridad/escribir` **y** `ventas/escribir`: nadie gana por esta vía una capacidad que no tuviera ya en ambos módulos. Consecuencia contraintuitiva: estas acciones usan el cliente cookie-bound y **no** `createAdminClient()` como el resto de `security/`, porque con la clave de servicio `auth.uid()` es NULL y el chequeo rebota. `configuracion` se lee con `seguridad/leer` **o** `ventas/leer` (el ABM de ítems necesita el pct y la fila no guarda secretos) y sólo se escribe con `seguridad/escribir`; no tiene políticas de INSERT ni DELETE, así que con RLS activa nadie puede borrar la fila ni crear una segunda.
  - **Revocar `EXECUTE` de `PUBLIC` no alcanza en Supabase**: los default privileges del schema `public` conceden `ALL ON FUNCTIONS` a `anon` y `authenticated`, y esos grants sobreviven al `REVOKE ... FROM PUBLIC` — el helper interno quedaba publicado como endpoint de PostgREST. Hay que nombrar los roles. Los ACLs finales quedan iguales a los de `registrar_venta`.
  - **El formulario de ítems lee el porcentaje configurado** en lugar del 20% fijo, texto de ayuda incluido ("socio + 25%", y copy propio cuando el recargo es 0). El pct **se congela al abrir el modal** en un ref y no entra como dependencia de ningún efecto: si entrara, su llegada tardía —o un cambio de configuración con el modal abierto— volvería a disparar el autocompletado y le pisaría la tarifa guardada al ítem en edición, que es el mismo bug que ya evitaba `recienReseteadoRef` para el cambio de ítem. Si la configuración no se puede leer, la acción devuelve 20 en vez de fallar: una config caída degrada al comportamiento previo en lugar de romper el ABM.
  - La fila guarda **quién corrió la reconstrucción, cuándo y sobre cuántos ítems**, y la pantalla lo muestra sobre el botón. No es un historial —no se pidió uno— pero cuesta tres columnas en la misma fila y es lo que evita la doble corrida. El botón queda deshabilitado mientras haya cambios sin guardar en el porcentaje: el RPC lee el valor de la base, no el del input, y sin ese guard el preview no se correspondería con lo que el operador tiene en pantalla.

- **Etiquetado automático de PRs escritos por agentes y reporte de métricas de entrega** (`.github/workflows/`, `scripts/agent-metrics/`). Primer contenido de `.github/` en el repo: hasta ahora no había ningún workflow, y el CI eran las apps de Vercel y CodeRabbit sin configuración versionada. Node sin dependencias (`.mjs` con `fetch` global): un `scripts/*.ts` habría entrado en `npx tsc --noEmit`, porque el `include` del `tsconfig.json` es `**/*.ts` con settings pensados para el bundler de Next.
  - **El tagger usa `pull_request`, no `pull_request_target`.** Lee tres strings que controla quien abre el PR (rama, cuerpo, mensajes de commit); `pull_request_target` le daría a ese job token de escritura y acceso a los secrets, que es la clase de CVE canónica de Actions. Lo único que cruza el shell es el **número** de PR, que lo pone GitHub — el resto lo re-consulta el script por la API. Los PRs de forks (hoy, ninguno) reciben token de solo lectura y los etiqueta después el workflow programado, que corre en contexto de la rama base.
  - **Cuatro señales de detección** — trailer de co-autoría en el *cuerpo* del commit, prefijo de rama, marcador en el cuerpo del PR y actor — y se registra cuál disparó, como etiquetas `agent:*`. La detección por casilla exige el sentinel literal `agent-authored`: la plantilla de PR tiene más arriba una casilla `agent-browser`, y un regex más laxo habría clasificado como escrito-por-agente a todo PR que hubiera pasado por el navegador. La reconciliación es idempotente y acotada al namespace `{agent-authored, human-authored, agent:*}`: si no hay nada que cambiar no emite **ninguna** escritura, que es lo que permite correr el tagger en cada `synchronize` sin ensuciar el timeline.
  - **Las métricas están construidas para no mentir con esta muestra.** Con 11 PRs auto-mergeados (91% a menos de 5 minutos de abrirse) y cero reverts, un reporte ingenuo diría *"cycle time 0.5min · CFR 0% · revert rate 0%"*: cuatro números correctos que describen la ausencia de proceso como si fuera excelencia. Por eso `n` va pegado a cada estadística, con `n < 5` se imprimen los valores crudos en vez de percentiles, los percentiles son *nearest-rank* y no interpolados, y una tasa de cero se reporta con su cota superior por regla de tres (`0/11 → ≤ ~27%`) en vez de `0%`. El reporte encabeza con **lead time a producción** —el único reloj no degenerado acá, porque el deploy de Vercel es trabajo real posterior al merge— y rotula `open→merge` como artefacto de auto-merge.
  - **Dos falsos positivos se encontraron calibrando contra los PRs reales, no razonando.** (1) `chatgpt-codex-connector` comenta a los segundos de abrirse cada PR, y el comentario dice *"You have reached your Codex usage limits for code reviews"* — un error de cuota que se contaba como feedback y hacía que "revisado antes del merge" diera 91% en un repo donde el valor real es 1 de 11. Se corrigió: un bot nunca hace feedback por `IssueComment`; su revisión real entra por `PullRequestReview` o por hilo inline. (2) La heurística de hotfix marcó 5 de 11 PRs, y al mirarlos uno por uno casi todos eran desarrollo secuencial sobre los mismos archivos (#2→#3, #3→#9), más el caso de #8 y #9 que **comparten la rama `feat/eliminar-catalogos`** y por lo tanto son el mismo trabajo. Se agregó el guard de misma-rama y la señal quedó **fuera del CFR de portada** (opt-in con `--count-hotfix`): publicar un 45% construido sobre eso es el mismo pecado que publicar un 0%. Las banderas se muestran igual, como `suspected`, con su evidencia y su score.
  - **Projects v2 no tiene dónde guardar agregados.** Los campos son estrictamente por ítem: no hay rollup, ni fórmula, ni API de GraphQL para los gráficos de "Insights" (son solo UI). Los datos por PR van como campos del tablero; las tasas y percentiles van al README del proyecto, a un issue de seguimiento agregado como tarjeta, y al job summary de Actions — que es el único que no necesita PAT y del que todo lo demás es una proyección. El sync degrada sin fallar si `PROJECTS_TOKEN` falta o le sobra scope (`INSUFFICIENT_SCOPES` es fallo blando). El PAT se expone **solo** en el workflow programado, nunca en el tagger.
  - `--dry-run` se aplica dentro del cliente HTTP, no en cada caller: lee de la API real y cortocircuita toda escritura. Verificado contra los 11 PRs mergeados a `main` — el reporte corre completo con `writes: 0`.
  - **El sistema reportará 0% de PRs de agente hasta que se active el trailer de co-autoría y el convenio de ramas `claude/*`.** Mide una práctica que todavía no deja rastro; el README del script documenta ese bootstrap junto con el PAT (tiene que ser **clásico** con scope `project`: los fine-grained no tienen permiso de Projects a nivel de cuenta, sólo de organización, así que no pueden escribir un tablero de usuario).
...
- **El padrón se truncaba en 1000 filas, en silencio** (P13.1). `getPadron()` hacía `.select()` sin `.range()`; PostgREST corta en `max_rows` y no falla. Con ~8.400 socios, la pantalla mostraba, contaba, exportaba e imprimía menos de un octavo del club. Ahora pagina con `fetchAllRows` sobre el RPC. Un padrón truncado no es incompleto: es falso.
- **El padrón incluía a los socios en categoría `Inactivo`** (P13.1). Excluía la baja buscando el nombre literal `'BAJA'` en vez de usar `categorias_sociales.cuenta_como_activo`, el predicado canónico desde `20260806000001`. Verificado en browser: el listado general quedó en 45 socios, sin `Inactivo`.
- **Un error al cargar el padrón se veía igual que un padrón legítimamente vacío** (P13.1). `fetchData` tenía `try/finally` sin `catch`: con el RPC lanzando `sin_permiso` —alcanzable de verdad en bootstrap mode— la pantalla mostraba "Sin resultados." y "Total: 0 habilitados a votar", y ese cero se podía exportar e imprimir. Ahora hay estado de error explícito, toast que distingue falta de permiso de error genérico, y export/impresión bloqueados mientras dure.
- **El corte impreso podía afirmar algo falso** (P13.1). Se leía de `socios[0]`, así que un filtro sin resultados imprimía "sin cuotas sociales emitidas a la fecha" —una afirmación sobre el club, en una hoja que se firma— cuando lo único cierto era que no había filas. Ahora viene de un RPC propio, independiente del listado.
- **La impresión podía salir con 100 filas y un total de 8.400** (P13.1). `handlePrint` asumía que `window.print()` bloquea hasta capturar la hoja; donde retorna antes (iOS Safari, webviews) el listado completo ya se había desmontado. Pasa a `beforeprint`/`afterprint`, que además cubre el `Ctrl+P` nativo — que antes no pasaba por el handler en absoluto. La hoja dejó además de salir con la barra de tabs del workspace y con el título duplicado.
- **Carrera entre respuestas del padrón** (P13.1). Con ~10 round-trips paginados por carga, cambiar el toggle o la categoría dos veces seguidas dejaba que la respuesta vieja resolviera última: las filas eran de un filtro y el encabezado, los criterios y el corte de otro. Se descarta todo lo que no sea la petición vigente.

- **Ítems dados de baja en el sistema viejo aparecían en el POS** (migración `20260808000001`). La tabla legacy `ItemsVentas` no tenía columna `activo`, así que la baja de un ítem se hacía **renombrándolo** con el prefijo `(DESUSO)` — el log de auditoría de Scriptcase registra esos renames uno por uno. `migration/migrate.py` copió el nombre tal cual y hardcodeó `activo = true` para las 210 filas, con lo cual 24 ítems `(DESUSO)` más uno `No Usar` seguían ofreciéndose en el combobox de `/ventas/nueva` y en las altas de movimientos de stock. Se marcan inactivos en las dos tablas (25 filas en `items_ventas`, 4 en `stock_items`) y `mig_items` pasa a derivar el flag del nombre, para que una re-corrida desde el dump no los reintroduzca.
  - El patrón es `ILIKE '%desuso%'` y no `LIKE '(DESUSO)%'`: hay un ítem con espacio inicial (`' (DESUSO) Fichas Helices - NO Socios - ESCOPETA'`) y varios sin espacio tras el paréntesis (`'(DESUSO)Id. Portacion...'`) que un patrón anclado al principio dejaría afuera. Verificado contra los 210 nombres del dump: 25 matches, cero falsos positivos.
  - **El nombre no se toca.** Las ventas históricas lo referencian y el reporte por-ítem incluye inactivos a propósito, así que los ítems siguen visibles en el ABM y en los reportes, con el badge `(inactivo)` que los combobox ya renderizaban.
  - **Efecto conocido, sobre un solo ítem.** De los 4 que se desactivan en `stock_items`, tres tienen existencia cero; el cuarto, `(DESUSO) Derecho de Linea NO Socios Arma Corta`, conserva 6479 unidades (142 en Secretaría, 6337 en Arma Corta). Al quedar inactivo ese saldo pasa a ser **visible pero inmovible**: no solo desaparece de los selectores de transferencias y movimientos, sino que `registrar_movimiento_stock` lo rechaza con *"El ítem no existe o está inactivo"* (`20260803000002:78-80`). Sigue apareciendo en Inventario —`getInventario()` no filtra por `activo`— y se destraba reactivándolo desde `/stock/items`. Se deja así a propósito: es un conteo fantasma —un "derecho de línea" es un servicio, no mercadería, y el ítem no registra una sola línea de venta en todo el histórico legacy—, así que ponerlo en cero sería inventar un ajuste de stock que nadie pidió.
  - Anular ventas viejas de estos ítems **sigue funcionando**: `anular_venta` restituye stock desde `movimientos_stock` sin chequear `activo`. Y `registrar_venta` ya rechazaba ítems inactivos (`JOIN items_ventas iv ON ... AND iv.activo`), así que la baja queda cubierta también del lado de la base, no solo del combobox.
...
- **El padrón electoral estaba imprimiendo el DNI de los socios, y se pegó así en el club.** La hoja salía con DNI, categoría, fecha de alta, localidad, edad y antigüedad debajo de cada nombre —tres renglones por socio— pese a que el cambio anterior existía justamente para evitarlo. Dos causas apiladas: la regla de densidad `[data-padron-compacto] tbody td { display: block }` es (0,1,2) y le gana a la utilidad `.print\:hidden` de Tailwind (0,1,0); y el `!important` que se había agregado para taparlo **nunca aplicó**, porque styled-jsx no colapsa el escape del template literal y emitía `td.print\\:hidden` con dos backslashes — un selector inválido que el navegador descarta entero. Se detectó leyendo el `<style>` de la página desplegada: la regla no existía en producción. **La verificación previa había sido defectuosa**: se probó contra una réplica donde el escape se resolvía a mano asumiendo semántica de template literal, o sea que se validó la suposición y no el código.
  - **El arreglo saca el escape del problema**: las celdas llevan `data-print-hide`, convención que el repo ya usa (`WorkspaceTabs`) y que ya tiene su regla `!important` en el mismo bloque. Un atributo no necesita escaparse en ningún pipeline de CSS-in-JS, así que la clase de bug desaparece por construcción.
  - **Y el ocultamiento deja de depender de la cascada.** Durante la impresión las columnas marcadas directamente no se renderizan (`renderColumns`, sobre el flag `printAll` que ya seteaba `beforeprint`). La regla CSS queda como respaldo para navegadores donde `beforeprint` no dispara. Se filtró dos veces por la misma vía; que el DNI llegue al papel ya no depende de que una regla gane una pelea de especificidad.
  - **La línea "Listado parcial" publicaba el término buscado.** El buscador matchea por DNI —lo dice el propio placeholder—, así que buscar un DNI para chequear si alguien vota e imprimir dejaba ese DNI en la pared, encima en `text-sm`, más grande que los nombres a 8pt. Ahora dice sólo "con búsqueda aplicada": la advertencia de listado parcial se conserva, el término no.
  - Fuente a **8pt** por pedido del club y pista del número de socio a 12mm (en producción llega a 5 dígitos). Medido en PDF real a A4: **65 páginas, ~129 nombres por hoja** (contra las 495 de la tabla de pantalla).

### Changed

- **Padrón electoral: la hoja impresa se publica, así que sale mínima.** El listado de habilitados a votar se pega en los distintos sectores del club, pero se imprimía con la tabla de pantalla entera —9 columnas, DNI y edad de todos los socios incluidos— más un encabezado con total, filtros y criterios. Exponer DNI y fecha de nacimiento de miles de socios en un pasillo es publicar datos personales que nadie necesita para encontrarse en la lista. Con el toggle "Solo habilitados a votar" activo, la impresión pasa a **Nro Socio, Apellido y Nombre**, con encabezado de título y fecha de emisión nada más. El orden por apellido ya era el vigente y no se tocó: viene del `ORDER BY b.apellido, b.nombre, b.nro_socio` del RPC `get_padron`, cuyo desempate por `nro_socio` es load-bearing para el paginado de `fetchAllRows`.
  - **El recorte es sólo de la hoja impresa.** Se implementa con una bandera `printHidden` por columna (`print:hidden`, que en `@media print` es `display: none`) y no filtrando el array `columns`: de ahí salen también los `exportHeaders`, así que sacar columnas habría achicado en silencio el Excel y el CSV — justo los que permiten auditar *por qué* alguien quedó habilitado. Verificado capturando el blob del export: sigue trayendo las 9 columnas. La impresión del padrón general (toggle apagado) no cambia, DNI incluido.
  - **La hoja avisa cuando es parcial.** Si hay categoría o búsqueda activa, el encabezado imprime "Listado parcial: N habilitados · …". Sin eso, buscar "Gonz" e imprimir publicaba 40 nombres bajo el título "Padrón Electoral" sin ninguna señal de que faltaban los demás.
  - **Arreglo de paso:** la línea "N habilitados no tienen cuotas sociales emitidas" no tenía `print:hidden` —sus dos hermanas sí—, así que se imprimía. Al quedar como el único número de la hoja se leía como "N socios deben cuotas": una métrica interna de calidad de datos, sin contexto, en una pared. Lo detectaron los dos revisores del gate por separado.
  - **Densidad: 495 hojas → 73.** Medido con 8.400 socios renderizando el PDF a A4 y contando páginas. La tabla de pantalla (`td p-4` = 16px arriba y abajo, `th h-12`) daba ~16 nombres por hoja. El grueso del desperdicio no era el cuerpo de letra sino el padding **y la mitad derecha de la hoja vacía**: tres columnas de texto corto no llenan un A4 a lo ancho. Bajando padding y fuente a 9pt son 156 páginas; en **dos columnas de nombres por hoja**, 73 — ~115 nombres por página. 9pt es el piso legible de parado a un brazo de distancia (8pt entraba en ~62 páginas pero ya cuesta leerlo en una pared). Se pierde la fila de encabezado repetida: un `thead` no se puede repetir arriba de cada columna, y se cambió por las hojas. Todo scopeado a `[data-padron-compacto]`, que sólo se pone con el toggle activo — el padrón general se imprime como siempre.
  - **La densidad reintroducía el DNI en la hoja, y no se veía en pantalla.** `[data-padron-compacto] tbody td { display: block }` tiene especificidad (0,1,2) y la utilidad `.print\:hidden` de Tailwind (0,1,0): el `display:block` le ganaba y las seis columnas ocultas volvían a imprimirse, encima sin encabezado que las identificara. Se detectó contando las celdas dibujadas en el PDF (3 en vez de 2) y se cierra reafirmando `display: none !important` sobre las celdas ocultas. Dos escollos más de la misma familia: dentro del template literal de `<style jsx global>` el `\:` se colapsa a `:` y dejaba el selector inválido (hacen falta dos barras), y las pistas del grid se derivan de `columns.filter(c => !c.printHidden)` en vez de un `3` hardcodeado, para que agregar una columna no desalinee la hoja en silencio.
